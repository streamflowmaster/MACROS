...
from load_models import load_CNMR_GPT
...
config_path = 'cnmr_config.yaml'
config = yaml.load(open(config_path, 'r'), Loader=yaml.FullLoader)
model,_ = load_CNMR_GPT(config=config,relative_dir='')


delta_embedding = model.transformer.delta_embedding.weight.detach().cpu().numpy()
vocab_size, emb_dim = delta_embedding.shape
print(f"Intensity 词汇表大小: {vocab_size}, 嵌入维度: {emb_dim}")

# L2 归一化，便于余弦相似度计算
embeddings_norm = normalize(delta_embedding, axis=1)
...
plt.savefig('random_cnmr_tokenization.svg', dpi=300)
plt.show()
