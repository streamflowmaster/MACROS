...
class BERT(nn.Module):
    ...
    def forward(self, x):
        ...
        b, n, l = x.shape
        assert n+self.config.n_learnable_tokens <= self.config.block_size, f"length {n} of sequence is greater than block_size {self.config.block_size}"
        ...
        # h = h + self.transformer.wpe(
        #     torch.arange(self.config.block_size, device=x.device)[None, :n])
        #
        h = torch.cat((learnable_tokens, h), dim=1)
        # position embedding
        h = h + self.transformer.wpe(torch.arange(self.config.block_size, device=x.device)[None, :n+self.config.n_learnable_tokens])
