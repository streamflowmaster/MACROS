...
from MultiSpec2Mol_MolPrompt_Pubchem.model import BERT, BERTConfig
from spectra_prdiction.model import GPT as SpectraGPT, GPTConfig as SpectraConfig
from MultiSpec2Mol_MolPrompt_Pubchem.cascade import MultiSpec2Mol
...
def load_HSQCNMR_GPT(config: dict,relative_dir:str):
    ...
    if init_from == 'scratch':
        ...
        print("Initializing a new HSQC model from scratch")
...
def load_IR_GPT(config: dict,relative_dir:str):
    ...
    if init_from == 'scratch':
        # init a new model from scratch
        print("Initializing a new IR model from scratch")
        # determine the vocab size we'll use for from-scratch training
...
def load_HNMR_GPT(config: dict,relative_dir:str):
    ...
    if init_from == 'scratch':
        # init a new model from scratch
        print("Initializing a new HNMR model from scratch")
        # determine the vocab size we'll use for from-scratch training
...
def load_CNMR_GPT(config: dict,relative_dir:str, use_cnmr_intensity = True):
    if relative_dir == '':
        out_dir = config['out_dir']
    else:
        out_dir = os.path.join(relative_dir,config['out_dir'])
...
def load_MolGPT(config: dict,relative_dir:str):
    ...
    if init_from == 'scratch':
        # init a new model from scratch
        print("Initializing a new Mol model from scratch")
        # determine the vocab size we'll use for from-scratch training
...
def load_forward(output_dir:str,
    ...
    if os.path.exists(proj_path):
        ...
        model.load_state_dict(torch.load(proj_path, map_location=device))
