...
if __name__ == '__main__':
    ...
    pipeline = MoleculeInferencePipeline(
        ...
        spec2mol_dir_refine='SPEC2Mol_Multi_Refine',
    ...
    for batch_idx in range(0, len(src), batch_size):
            ...
            # Checkpoint saving
            if current_batch_num % checkpoint_interval == 0 or batch_idx + batch_size >= len(data):
                checkpoint_count += 1
                checkpoint_file = (f"{type}_{checkpoint_prefix}_"
                                   f"{checkpoint_count}_sample_width{sample_width}_"
                                   f"select_ratio{select_ratio}_"
                                   f"cycles_num_{num_refine_select_cycles}.xlsx")
                ...
                # print(
                #     f"Checkpoint saved to {checkpoint_file} after processing {samples_processed} samples (save time: {format_time(checkpoint_save_time)})")
